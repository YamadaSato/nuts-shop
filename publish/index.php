...
foreach ($sql as $row) {
 ...
 if (file_exists($filename)) {
	...
	<p>
	<a href="detail.php?id=<?=$id?>"><img src="image/<?=$a?>.jpg"></a>
	</p>
	<?php
	}else{
		...
		?>
		<p>
		<a href="detail.php?id=<?=$id?>"><img src="admin/<?=$u[1]?>"></a>
	</p>
<?php
	}?>
...
<?php
		$sql =	$pdo->query( "SELECT count(*) as ct FROM product");

				// 最初の1行だけ取り出す
		$count = $sql->fetch()['ct']; // → 14

				//1ページの件数で割って切り上げる
		$count = ceil($count / $item_count); // → 
				// 1から始めた方が効率がいい
		for( $i=1 ;$i <= $count ; $i ++ ){
?>
